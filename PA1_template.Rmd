...
```{r}
require(lubridate)
dataimp$weekday <- wday(dataimp$date, label = T)
mean_steps_per_interval_weekdays <- dataimp[(dataimp$weekday %in% levels(dataimp$weekday)[2:6]),] %>%
  group_by(interval) %>%
  summarize(mean_steps = round(mean(steps, na.rm = TRUE)))
mean_steps_per_interval_weekends <- dataimp[(dataimp$weekday %in% levels(dataimp$weekday)[c(1,7)]),] %>%
  group_by(interval) %>%
  summarize(mean_steps = round(mean(steps, na.rm = TRUE)))
plot(mean_steps_per_interval_weekdays, type = "l", main = "Average daily activity pattern for weekdays", ylab = "Average steps per interval", xlab = "Interval", ylim=c(0,250))
plot(mean_steps_per_interval_weekends, type = "l", main = "Average daily activity pattern for weekends", ylab = "Average steps per interval", xlab = "Interval", ylim=c(0,250))
```
 
>*We can see the differences in activity patterns between weekdays and weekends. At weekends there are less average steps during the morning (7-9) than at weekdays but there are more steps during the day.*
